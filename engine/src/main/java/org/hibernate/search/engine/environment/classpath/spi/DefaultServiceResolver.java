...
public abstract class DefaultServiceResolver implements ServiceResolver {
	...
	private static class ClassPathAndModulePathServiceResolver extends DefaultServiceResolver {
		...
		private <S> Iterator<? extends Supplier<S>> providerIterator(ClassLoader classLoader, Class<
				S> serviceContract) {
			try {
		 ...
		 */
		private <S> void collectServiceIfNotDuplicate(Set<S> services, Set<String> alreadyUsedProviderTypes, Supplier<
				S> provider) {
			Class<?> type;
			try {
