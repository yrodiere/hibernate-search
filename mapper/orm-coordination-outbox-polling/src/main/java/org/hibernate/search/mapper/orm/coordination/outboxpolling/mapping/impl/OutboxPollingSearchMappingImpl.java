...
public class OutboxPollingSearchMappingImpl implements OutboxPollingSearchMapping {
	...
	private static final String COUNT_EVENTS_WITH_STATUS = "select count(e) from "
			+ ENTITY_NAME
			+ " e where e.status = :status";
	private static final String UPDATE_EVENTS_WITH_STATUS = "update "
			+ ENTITY_NAME
			+ " e set e.status = :newStatus where e.status = :status";
