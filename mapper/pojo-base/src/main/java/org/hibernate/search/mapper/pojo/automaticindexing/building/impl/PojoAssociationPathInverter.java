...
final class PojoAssociationPathInverter {
	...
	private void collectAssociationPathsToMatch(
		...
		if ( parentValuePath != null ) {
			...
			if ( isDefaultExtractorPath ) {
				...
				associationPathsToMatch.add( PojoModelPath.ofValue( propertyName, ContainerExtractorPath
						.defaultExtractors() ) );
			}
		}
	}
	...
	private Optional<PojoModelPathValueNode> findInverseSidePathFromOriginalSide(
		...
		Optional<PojoModelPathValueNode> result = propertyAdditionalMetadata.getValueAdditionalMetadata( pathToInvert
				.getExtractorPath() )
				.getInverseSidePath();
		if ( result.isPresent() ) {
			return result;
