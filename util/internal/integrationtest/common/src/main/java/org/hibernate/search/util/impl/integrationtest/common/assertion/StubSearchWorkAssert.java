...
public class StubSearchWorkAssert {
	...
	private final StubTreeNodeDiffer<StubProjectionNode> PROJECTION_DIFFER = StubTreeNodeDiffer
			.<StubProjectionNode>builder().build();
	...
	public StubSearchWorkAssert matches(StubSearchWork expected) {
		...
		mismatch = checkForMismatch( builder, "truncateAfterTimeout", expected.getTruncateAfterTimeout(), actual
				.getTruncateAfterTimeout() );
		hasAnyMismatch = hasAnyMismatch || mismatch;

		mismatch = checkForMismatch( builder, "truncateAfterTimeUnit", expected.getTruncateAfterTimeUnit(), actual
				.getTruncateAfterTimeUnit() );
		hasAnyMismatch = hasAnyMismatch || mismatch;

		mismatch = checkForMismatch( builder, "failAfterTimeout", expected.getFailAfterTimeout(), actual
				.getFailAfterTimeout() );
		hasAnyMismatch = hasAnyMismatch || mismatch;

		mismatch = checkForMismatch( builder, "failAfterTimeUnit", expected.getFailAfterTimeUnit(), actual
				.getFailAfterTimeUnit() );
		hasAnyMismatch = hasAnyMismatch || mismatch;
