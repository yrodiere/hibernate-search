...
public class ElasticsearchSearchHighlighterImpl implements ElasticsearchSearchHighlighter {
	...
	public static final ElasticsearchSearchHighlighter NO_OPTIONS_CONFIGURATION =
			new ElasticsearchSearchHighlighterImpl(
					Collections.emptySet(), null, null, null, null, null, null, null, null, null, null, null, null,
					null, null, null
			);

	private static final JsonObjectAccessor REQUEST_HIGHLIGHT_ACCESSOR = JsonAccessor.root().property( "highlight" )
			.asObject();
	private static final JsonAccessor<String> TYPE = JsonAccessor.root().property( "type" ).asString();
	private static final JsonAccessor<String> BOUNDARY_CHARS = JsonAccessor.root().property( "boundary_chars" )
			.asString();
	private static final JsonAccessor<Integer> BOUNDARY_MAX_SCAN = JsonAccessor.root().property( "boundary_max_scan" )
			.asInteger();
	private static final JsonAccessor<String> ENCODER = JsonAccessor.root().property( "encoder" ).asString();
	private static final JsonAccessor<Integer> FRAGMENT_SIZE = JsonAccessor.root().property( "fragment_size" )
			.asInteger();
	private static final JsonAccessor<Integer> NO_MATCH_SIZE = JsonAccessor.root().property( "no_match_size" )
			.asInteger();
	private static final JsonAccessor<Integer> NUMBER_OF_FRAGMENTS = JsonAccessor.root().property(
			"number_of_fragments" ).asInteger();
	...
	private static final JsonAccessor<String> BOUNDARY_SCANNER = JsonAccessor.root().property( "boundary_scanner" )
			.asString();
	private static final JsonAccessor<String> BOUNDARY_SCANNER_LOCALE = JsonAccessor.root().property(
			"boundary_scanner_locale" ).asString();
	private static final JsonAccessor<String> FRAGMENTER = JsonAccessor.root().property( "fragmenter" ).asString();
	private static final JsonAccessor<Integer> PHRASE_LIMIT = JsonAccessor.root().property( "phrase_limit" )
			.asInteger();
