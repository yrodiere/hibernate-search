...
public class ElasticsearchRequestSuccessAssessor {
	...
	private static final JsonAccessor<Integer> BULK_ITEM_STATUS_CODE = JsonAccessor.root().property( "status" )
			.asInteger();
	private static final JsonAccessor<String> ERROR_TYPE = JsonAccessor.root().property( "error" ).property( "type" )
			.asString();
	...
	public void checkSuccess(JsonObject bulkResponseItem) {
		...
		JsonObject responseBody = bulkResponseItem == null ?
				null : bulkResponseItem.entrySet().iterator().next().getValue().getAsJsonObject();
