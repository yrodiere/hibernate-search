...
public class ElasticsearchDefaultWorkBulkerTest {
	...
	public void alwaysBulk() {
		...
		when( work1.getRefreshStrategy() ).thenReturn( DEFAULT_REFRESH );
		when( sequenceBuilderMock.addBulkExecution( bulkWorkFutureArgumentCaptor.capture() ) ).thenReturn(
				bulkWorkResultFuture );
		when( sequenceBuilderMock.addBulkResultExtraction( bulkWorkResultFuture, work1, 0 ) )
				.thenReturn( work1FutureFromSequenceBuilder );
		bulker.add( work1 );
		verifyNoOtherSequenceInteractionsAndReset();
	...
	public void newBulkOnTooManyBulkedWorks() {
		...
		when( sequenceBuilderMock.addBulkExecution( bulkWork1FutureArgumentCaptor.capture() ) ).thenReturn(
				bulkWork1ResultFuture );
		for ( int i = 0; i < DEFAULT_MAX_BULK_SIZE; ++i ) {
		...
		when( sequenceBuilderMock.addBulkExecution( bulkWork2FutureArgumentCaptor.capture() ) ).thenReturn(
				bulkWork2ResultFuture );
		...
		when( bulkWorkFactoryMock.apply( Arrays.asList( additionalWork1, additionalWork2 ), DEFAULT_REFRESH ) )
				.thenReturn(
						bulkWork2 );
		bulker.finalizeBulkWork();
		verifyNoOtherSequenceInteractionsAndReset();
	...
	public void newBulkOnDifferentRefresh() {
		...
		when( bulkWorkFactoryMock.apply( Arrays.asList( work3, work4 ), DocumentRefreshStrategy.FORCE ) ).thenReturn(
				bulkWork2 );
		when( sequenceBuilderMock.addBulkExecution( bulkWork3FutureArgumentCaptor.capture() ) ).thenReturn(
				bulkWork3ResultFuture );
