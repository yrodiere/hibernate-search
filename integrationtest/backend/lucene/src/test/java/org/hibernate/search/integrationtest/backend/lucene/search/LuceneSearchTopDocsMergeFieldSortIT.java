...
public class LuceneSearchTopDocsMergeFieldSortIT<F> {
	...
	public void asc() {
		...
		LuceneSearchQuery<DocumentReference> segment0Query = matchNonEmptySortedByFieldQuery( dataSet, SEGMENT_0,
				SortOrder.ASC );
		LuceneSearchQuery<DocumentReference> segment1Query = matchNonEmptySortedByFieldQuery( dataSet, SEGMENT_1,
				SortOrder.ASC );
	...
	private LuceneSearchQuery<DocumentReference> matchNonEmptySortedByFieldQuery(DataSet<F> dataSet, int segment,
		...
		return scope.query().extension( LuceneExtension.get() )
				.where( f -> f.matchAll().except( f.id().matchingAny( Arrays.asList( dataSet.seg0EmptyDocId,
	...
	private static class DataSet<F> {
		...
		private DataSet(TestedFieldStructure fieldStructure, FieldTypeDescriptor<F> fieldType, SortMode sortModeOrNull,
			...
			String routingKeyBase = fieldType.getUniqueName()
					+ "_"
