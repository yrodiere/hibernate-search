...
public class ObjectStructureIT {
	...
	public void index_error_invalidFieldForDocumentElement_flattened() {
		assertThatThrownBy( () -> index.index( "willNotWork", document -> document.addValue( index
				.binding().flattenedObject.string, "willNotWork" ) ) )
				.isInstanceOf( SearchException.class )
				.hasMessageContainingAll(
						"Invalid field reference for this document element",
	...
	public void index_error_invalidFieldForDocumentElement_nested() {
		assertThatThrownBy( () -> index.index( "willNotWork", document -> document.addValue( index
				.binding().nestedObject.string, "willNotWork" ) ) )
				.isInstanceOf( SearchException.class )
				.hasMessageContainingAll(
						"Invalid field reference for this document element",
	...
	private void initData() {
		index.bulkIndexer()
				...
				.add( EXPECTED_NON_NESTED_MATCH_ID, document -> {
					 ...
					 */
					for ( ObjectMapping objectMapping : Arrays.asList( index.binding().flattenedObject, index
							.binding().nestedObject ) ) {
