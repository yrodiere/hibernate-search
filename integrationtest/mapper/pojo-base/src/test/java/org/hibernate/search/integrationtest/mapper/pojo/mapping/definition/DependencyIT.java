	...
	public void derivedFrom_nonRoot_polymorphism_prevervesGenerics() {
		...
		class Model {
			...
			class ContainedEntity1<T extends OtherContainedSuperClass> extends AbstractContainedEntity<T> {
				@Override
				@IndexingDependency(derivedFrom = {
						@ObjectPath({ @PropertyValue(propertyName = "contained"), @PropertyValue(
	...
	public void derivedFrom_error_cycle_buried() {
		...
		assertThatThrownBy(
				() -> setupHelper.start()
						...
						.withAnnotatedTypes( DerivedFromCycle.A.class, DerivedFromCycle.B.class,
								DerivedFromCycle.C.class )
						.setup()
		)
				.isInstanceOf( SearchException.class )
				.satisfies( FailureReportUtils.hasFailureReport()
	...
	public void error_cannotInvertAssociation() {
		...
		assertThatThrownBy(
				...
				.satisfies( FailureReportUtils.hasFailureReport()
						...
						.pathContext(
								".embedded<no value extractors>.b<no value extractors>.text<no value extractors>" )
						.failure(
								...
								" Hibernate Search needs this information in order to reindex '"
										+ CannotInvertAssociation.A.class.getName()
										+ "' when '"
										+ CannotInvertAssociation.B.class.getName()
										+ "' is modified.",
								// Tips
								"@OneToMany(mappedBy",
		...
		assertThatThrownBy(
				...
				.hasMessageContaining(
						"This path was resolved as the inverse side of the association '.b<no value extractors>'"
								+ " on type '"
								+ CannotApplyInvertAssociationPath.A.class.getName()
								+ "'"
				)
				.hasMessageContaining(
