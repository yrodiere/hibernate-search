	...
	public void cleanup() {
		withinEntityManager( factory, em -> {
			withinTransaction( em, () -> {
				em.createQuery( "from " + POI.class.getName() ).getResultList().forEach( entity -> em.remove(
						entity ) );
				em.createQuery( "from " + DoubleIndexedPOI.class.getName() ).getResultList().forEach( entity -> em
						.remove( entity ) );
			} );
		} );
	}
		...
		withinEntityManager( factory, em -> {
			...
			withinTransaction( em, () -> {
				...
				org.apache.lucene.search.Query luceneQuery = builder.spatial().onField( "location" )
						.within( 100, Unit.KM ).ofLatitude( centerLatitude ).andLongitude( centerLongitude )
						.createQuery();
	...
	public void testDistanceProjectionWithoutDistanceSortMissingCoordinates() throws Exception {
		...
		withinEntityManager( factory, em -> {
			...
			withinTransaction( em, () -> {
				...
				org.apache.lucene.search.Query luceneQuery = builder.spatial().onField( "location" )
						.within( LARGE_DISTANCE_KM, Unit.KM ).ofLatitude( centerLatitude ).andLongitude(
	...
	public void testDistanceSortWithMaxResult() throws Exception {
		...
		withinEntityManager( factory, em -> {
			...
			withinTransaction( em, () -> {
				...
				org.apache.lucene.search.Query luceneQuery = builder.spatial().onField( "location" ).within( 100,
						Unit.KM ).ofLatitude( centerLatitude )
	...
	public void testDoubleIndexedDistanceProjection() throws Exception {
		...
		withinEntityManager( factory, em -> {
			...
			withinTransaction( em, () -> {
				...
				final QueryBuilder builder = em.getSearchFactory().buildQueryBuilder().forEntity(
						DoubleIndexedPOI.class ).get();
